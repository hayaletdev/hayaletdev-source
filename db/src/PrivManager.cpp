...
void CPrivManager::Update()
{
	...
	while (!m_pqPrivGuild.empty() && m_pqPrivGuild.top().first <= now)
	{
		...
		{
			PrivGuildDataMap::iterator it = m_aPrivGuild[p->type].find(p->guild_id);
	...
	while (!m_pqPrivChar.empty() && m_pqPrivChar.top().first <= now)
	{
		...
		if (!p->bRemoved)
		{
			...
			PrivCharDataMap::iterator it = m_aPrivChar[p->type].find(p->pid);
...
void CPrivManager::AddCharPriv(DWORD pid, BYTE type, int value)
{
	...
	PrivCharDataMap::iterator it = m_aPrivChar[type].begin();
...
void CPrivManager::SendPrivOnSetup(CPeer* peer)
{
	for (int i = 1; i < MAX_PRIV_NUM; ++i)
	{
		...
		for (PrivGuildDataMap::iterator it = m_aPrivGuild[i].begin(); it != m_aPrivGuild[i].end(); ++it)
		{
			// ADD_GUILD_PRIV_TIME
		...
		for (PrivCharDataMap::iterator it = m_aPrivChar[i].begin(); it != m_aPrivChar[i].end(); ++it)
		{
