...
namespace marriage
{
	...
	bool CManager::Initialize()
	{
		...
		snprintf(szQuery, sizeof(szQuery),
			"SELECT `pid1`, `pid2`, `love_point`, `time`, `is_married`, p1.`name`, p2.`name` FROM marriage%s, player%s AS p1, player%s AS p2 WHERE p1.`id` = `pid1` AND p2.`id` = `pid2`",
			GetTablePostfix(), GetTablePostfix(), GetTablePostfix());
	...
	{
		DWORD now = static_cast<DWORD>(CClientManager::instance().GetCurrentTime());
		...
		char szQuery[512];
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO marriage%s (`pid1`, `pid2`, `love_point`, `time`) VALUES (%u, %u, 0, %u)", GetTablePostfix(), dwPID1, dwPID2, now);
		...
		{
			sys_log(0, "cannot update marriage : PID:%u %u", dwPID1, dwPID2);
			return;
		}
	...
	void CManager::Update(DWORD dwPID1, DWORD dwPID2, INT iLovePoint, BYTE byMarried)
	{
		...
		char szQuery[512];
		snprintf(szQuery, sizeof(szQuery), "UPDATE marriage%s SET `love_point` = %d, `is_married` = %d WHERE `pid1` = %u AND `pid2` = %u",
			GetTablePostfix(), iLovePoint, byMarried, pMarriage->pid1, pMarriage->pid2);
	...
	void CManager::Remove(DWORD dwPID1, DWORD dwPID2)
	{
		...
		char szQuery[512];
		snprintf(szQuery, sizeof(szQuery), "DELETE FROM marriage%s WHERE `pid1` = %u AND `pid2` = %u", GetTablePostfix(), dwPID1, dwPID2);
	...
	void CManager::EngageToMarriage(DWORD dwPID1, DWORD dwPID2)
	{
		...
		char szQuery[512];
		snprintf(szQuery, sizeof(szQuery), "UPDATE marriage%s SET `is_married` = 1 WHERE `pid1` = %u AND `pid2` = %u",
			GetTablePostfix(), pMarriage->pid1, pMarriage->pid2);
	...
	{
		DWORD dwStartTime = static_cast<DWORD>(CClientManager::instance().GetCurrentTime());
