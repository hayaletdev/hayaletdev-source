				...
				if (pkAff)
				{
						...
						m_pkChr->GetPoint(POINT_STAT) != pkAff->lApplyValue)
					{
						...
						//if (pMap->IsFlagOnBase(idx))
						{
							...
							char buf[256];
							snprintf(buf, sizeof(buf), LC_STRING("%s 길드가 %s 길드의 깃발을 빼앗았습니다!", pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName()));
							pMap->Notice(buf);
						}
					}
				}
...
void CHARACTER::__StateIdle_NPC()
{
	...
	else
	{
		...
		if (GetRaceNum() == xmas::MOB_XMAS_SANTA_VNUM) // 산타
		{
...
void CHARACTER::StateBattle()
{
	...
	if (!victim->IsInSafezone() && !Attack(victim)) // 공격 실패라면? 왜 실패했지? TODO
		m_dwStateDuration = passes_per_sec / 2;
	else
	{
...
void CHARACTER::StateFlag()
{
	...
	snprintf(buf, sizeof(buf), LC_STRING("%s 길드의 깃발을 %s 님이 획득하였습니다.", pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName()));
	pMap->Notice(buf);
}
