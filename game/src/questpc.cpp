...
namespace quest
{
	...
	void PC::SendQuestInfoPakcet()
	{
		...
		{
			m_RunningQuestState->_title.reserve(QUEST_TITLE_NAME_LEN + 1);
			buf.write(m_RunningQuestState->_title.c_str(), QUEST_TITLE_NAME_LEN + 1);
			qi.size += QUEST_TITLE_NAME_LEN + 1;
		...
		{
			m_RunningQuestState->_clock_name.reserve(QUEST_CLOCK_NAME_LEN + 1);
			buf.write(m_RunningQuestState->_clock_name.c_str(), QUEST_CLOCK_NAME_LEN + 1);
			qi.size += QUEST_CLOCK_NAME_LEN + 1;
		...
		{
			m_RunningQuestState->_counter_name.reserve(QUEST_COUNTER_NAME_LEN + 1);
			buf.write(m_RunningQuestState->_counter_name.c_str(), QUEST_COUNTER_NAME_LEN + 1);
			qi.size += QUEST_COUNTER_NAME_LEN + 1;
		...
		{
			m_RunningQuestState->_icon_file.reserve(QUEST_ICON_FILE_NAME_LEN + 1);
			buf.write(m_RunningQuestState->_icon_file.c_str(), QUEST_ICON_FILE_NAME_LEN + 1);
			qi.size += QUEST_ICON_FILE_NAME_LEN + 1;
	...
	void PC::Reward(LPCHARACTER ch)
	{
		if (m_bIsGivenReward)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("<퀘스트> 이전에 같은 보상을 받은 적이 있어 다시 받지 않습니다."));
			m_bIsGivenReward = false;
		}
