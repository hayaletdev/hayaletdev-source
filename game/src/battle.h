#ifndef __INC_BATTLE_H__
#define __INC_BATTLE_H__
...
inline void AttackAffect(LPCHARACTER pkAttacker,
	...
	POINT_TYPE att_point,
	DWORD immune_flag,
	DWORD affect_idx,
	POINT_TYPE affect_point,
	long affect_amount,
	DWORD affect_flag,
	int time,
	const char* name)
{
	if (pkAttacker->GetPoint(att_point) && !pkVictim->IsAffectFlag(affect_flag))
	{
		...
		if (number(1, 100) <= (pkAttacker->IsSungMaCursed(POINT_SUNGMA_IMMUNE) ? pkAttacker->GetPoint(att_point) / 2 : pkAttacker->GetPoint(att_point)) && !pkVictim->IsImmune(immune_flag))
#else
