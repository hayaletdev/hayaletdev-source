#ifndef __INC_QUEST_MANAGER_H__
#define __INC_QUEST_MANAGER_H__
...
namespace quest
{
	...
	public:
		...
		unsigned int GetCurrentNPCEventRace();
#endif
		...
		void Damage(unsigned int pc, unsigned int npc);
#endif
#if defined(__QUEST_EVENT_DEAD__)
		void Dead(unsigned int pc, unsigned int npc);
#endif
#if defined(__QUEST_EVENT_FISH__)
		void Fishing(unsigned int pc);
		bool FishItem(unsigned int pc, LPITEM item);
#endif
		...
		void RestartHere(unsigned int pc);
#endif
#if defined(__QUEST_REQUEST_EVENT__)
		void RequestEvent(DWORD pc, DWORD quest_index, int state);
#endif
