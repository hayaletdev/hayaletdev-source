...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? LC_STRING("참") : LC_STRING("거짓"));
		}
	}

	pkChar->ChatPacket(CHAT_TYPE_INFO, LC_STRING("총 퀴즈 수: %d", c));
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	switch (flag)
	{
		...
		case 1:
		{
			SendControlNoticeMap("#start", OXEVENT_MAP_INDEX);
			SendControlNoticeMap(LC_STRING("정답은"), OXEVENT_MAP_INDEX);

			if (info->answer == true)
			{
				COXEventManager::instance().CheckAnswer(true);
				SendControlNoticeMap(LC_STRING("O 입니다"), OXEVENT_MAP_INDEX);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendControlNoticeMap(LC_STRING("X 입니다"), OXEVENT_MAP_INDEX);
			}

			SendControlNoticeMap(LC_STRING("5초 뒤 틀리신 분들을 바깥으로 이동 시키겠습니다."), OXEVENT_MAP_INDEX);
			SendControlNoticeMap(LC_STRING("다음 문제 준비해주세요."), OXEVENT_MAP_INDEX);
			SendControlNoticeMap("#send", OXEVENT_MAP_INDEX);
		...
		case 2:
		{
			...
			SendControlNoticeMap("#end", OXEVENT_MAP_INDEX);
			flag = 0;
		}
		break;
	...
	{
		case 0:
			SendNoticeMap(LC_STRING("10초뒤 판정하겠습니다."), OXEVENT_MAP_INDEX, true);
			flag++;
			return PASSES_PER_SEC(10);

		case 1:
			SendNoticeMap(LC_STRING("정답은"), OXEVENT_MAP_INDEX, true);

			if (info->answer == true)
			{
				COXEventManager::instance().CheckAnswer(true);
				SendNoticeMap(LC_STRING("O 입니다"), OXEVENT_MAP_INDEX, true);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendNoticeMap(LC_STRING("X 입니다"), OXEVENT_MAP_INDEX, true);
			}

			SendNoticeMap(LC_STRING("5초 뒤 틀리신 분들을 바깥으로 이동 시키겠습니다."), OXEVENT_MAP_INDEX, true);
		...
		case 2:
			COXEventManager::instance().WarpToAudience();
			COXEventManager::instance().SetStatus(OXEVENT_CLOSE);
			SendNoticeMap(LC_STRING("다음 문제 준비해주세요."), OXEVENT_MAP_INDEX, true);
			flag = 0;
			break;
	}
#endif
...
#if defined(__OX_RENEWAL__)
	SendControlNoticeMap(LC_STRING("문제 입니다."), OXEVENT_MAP_INDEX);
	SendControlNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX);
	SendControlNoticeMap(LC_STRING("맞으면 O, 틀리면 X로 이동해주세요"), OXEVENT_MAP_INDEX);
	SendControlNoticeMap(LC_STRING("The answer will appear in 20 seconds."), OXEVENT_MAP_INDEX);
	SendControlNoticeMap("#start", OXEVENT_MAP_INDEX);
#else
	SendNoticeMap(LC_STRING("문제 입니다."), OXEVENT_MAP_INDEX, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true);
	SendNoticeMap(LC_STRING("맞으면 O, 틀리면 X로 이동해주세요"), OXEVENT_MAP_INDEX, true);
#endif
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	MobKillCountMap::iterator iter = m_map_attender.begin();
	MobKillCountMap::iterator iter_tmp;
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_STRING("정답입니다!"));
