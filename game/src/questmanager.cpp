...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("fish", QUEST_FISH_ITEM_EVENT));
#endif
		...
		m_mapEventName.insert(TEventNameMap::value_type("restart_here", QUEST_RESTART_HERE_EVENT));
#endif
#if defined(__QUEST_REQUEST_EVENT__)
		m_mapEventName.insert(TEventNameMap::value_type("request", QUEST_REQUEST_EVENT));
#endif
	...
	int CQuestManager::ReadQuestCategoryFile(WORD quest_index)
	{
		...
		ifstream file((g_stQuestDir + "/questcategory.txt"));
		if (file.is_open())
		{
			string line;
			while (getline(file, line))
			{
				...
				{
					int type = stoi(line.substr(0, tab_pos));
					string name = line.substr(tab_pos + 1);
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/
			...
			if (npc >= MAIN_RACE_MAX_NUM) // Prevent duplicate QUEST_NO_NPC calls!
	...
	void CQuestManager::PartyKill(unsigned int pc, unsigned int npc)
	{
		...
		sys_log(0, "CQuestManager::PartyKill QUEST_KILL_EVENT (pc=%d, npc=%d)", pc, npc);
...
#if defined(__QUEST_EVENT_DAMAGE__)
	void CQuestManager::Damage(unsigned int pc, unsigned int npc)
	{
		PC* pPC;

		if (test_server)
			sys_log(0, "CQuestManager::Damage QUEST_DAMAGE_EVENT (pc=%d, npc=%d)", pc, npc);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[npc].OnDamage(*pPC);
			if (npc != QUEST_NO_NPC)
				m_mapNPC[QUEST_NO_NPC].OnDamage(*pPC);
		}
		else
			sys_err("QUEST: no such pc id : %d", pc);
	}
	...
	void CQuestManager::Dead(unsigned int pc, unsigned int npc)
	{
		...
		if (test_server)
			sys_log(0, "CQuestManager::Dead QUEST_DEAD_EVENT (pc=%d, npc=%d)", pc, npc);
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("퀘스트를 로드하는 중입니다. 잠시만 기다려 주십시오."));
	...
	unsigned int CQuestManager::GetCurrentNPCEventRace()
	{
		return GetCurrentNPCCharacterPtr() ? GetCurrentNPCCharacterPtr()->GetEventRaceNum() : 0;
	}
#endif
	...
	{
		auto it = m_mapEventFlag.begin();
		for (; it != m_mapEventFlag.end(); ++it)
		{
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
...
#if defined(__INGAME_EVENT_MANAGER__)
		CInGameEventManager::Instance().SetInGameEvent(name, value, prev_value);
#endif
		...
		{
			CInGameEventManager::instance().InitializeBanners();
		}
#endif
#if defined(__POPUP_NOTICE__)
		else if (name == "PopupNoticeEventFlag")
		{
			if (prev_value != value)
			{
				...
				for (auto it = c_rDescSet.begin(); it != c_rDescSet.end(); ++it)
				{
					LPCHARACTER ch = (*it)->GetCharacter();
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "PopupNoticeEventFlag %d", value);
				}
			}
		}
#endif
#if defined(__METINSTONE_SWAP__)
		else if (name == "metinstone_swap")
		{
			if (prev_value != value)
			{
				for (const DESC_MANAGER::DESC_SET::value_type& it : DESC_MANAGER::instance().GetClientSet())
				{
					const LPCHARACTER pChar = it->GetCharacter();
					if (NULL != pChar)
						pChar->ChatPacket(CHAT_TYPE_COMMAND, "metinstone_swap %d", value);
				}
			}
		}
#endif
...
#if defined(__POPUP_NOTICE__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("PopupNoticeEventFlag")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "PopupNoticeEventFlag %d", iEventFlagValue);
		}
#endif

#if defined(__METINSTONE_SWAP__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("metinstone_swap")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "metinstone_swap %d", iEventFlagValue);
		}
#endif

#if defined(__INGAME_EVENT_MANAGER__)
		CInGameEventManager::Instance().BroadcastInGameEventOnLogin(ch);
#endif
	}
	...
	void CQuestManager::Reload()
	{
		...
		for (auto it = m_registeredNPCVnum.begin(); it != m_registeredNPCVnum.end(); ++it)
		{
			char buf[256];
			DWORD dwVnum = *it;
			snprintf(buf, sizeof(buf), "%lu", dwVnum);
	...
	void CQuestManager::Fishing(unsigned int pc)
	{
		...
		{
			sys_err("QUEST FISHING_EVENT no such pc id : %d", pc);
		}
	}

	bool CQuestManager::FishItem(unsigned int pc, LPITEM item)
	{
		if (test_server)
			sys_log(0, "questmanager::FishItem Start : itemVnum : %d PC : %d", item->GetOriginalVnum(), pc);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			return m_mapNPC[item->GetVnum()].OnFishItem(*pPC);
		}
		else
		{
			sys_err("QUEST FISH_ITEM_EVENT no such pc id : %d", pc);
			return false;
		}
	}
#endif
...
#if defined(__QUEST_EVENT_RESTART_HERE__)
	void CQuestManager::RestartHere(unsigned int pc)
	{
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnRestartHere(*pPC);
		}
		else
		{
			sys_err("QUEST RESTART_HERE_EVENT no such pc id : %d", pc);
		}
	}
#endif

#if defined(__QUEST_REQUEST_EVENT__)
	void CQuestManager::RequestEvent(DWORD pc, DWORD quest_index, int state)
	{
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnRequestEvent(*pPC, quest_index, state);
		}
		else
			sys_err("QUEST REQUEST_EVENT no such pc id : %d", pc);
	}
#endif
}
