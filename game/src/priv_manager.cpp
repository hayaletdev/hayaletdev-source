...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		...
		{
			char buf[256]{};
			snprintf(buf, sizeof(buf), LC_STRING("%s 길드의 %s이 %d%% 증가했습니다!", g->GetName(), c_apszPrivNames[type], value));
			SendNotice(buf);
		}
		else
		{
			char buf[256]{};
			snprintf(buf, sizeof(buf), LC_STRING("%s 길드의 %s이 정상으로 돌아왔습니다.", g->GetName(), c_apszPrivNames[type]));
			SendNotice(buf);
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		char buf[256]{};
		snprintf(buf, sizeof(buf), LC_STRING("%s의 %s이 %d%% 증가했습니다!", c_apszEmpireNames[empire], c_apszPrivNames[type], value));
	...
	{
		char buf[256]{};
		snprintf(buf, sizeof(buf), LC_STRING("%s의 %s이 정상으로 돌아왔습니다.", c_apszEmpireNames[empire], c_apszPrivNames[type]));
