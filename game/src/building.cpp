...
CObject::CObject(TObject* pData, TObjectProto* pProto)
	: m_pProto(pProto), m_dwVID(0), m_chNPC(NULL), m_pkLand(NULL)
{
...
bool CObject::Show(long lMapIndex, long x, long y)
{
	...
	if (!tree)
	{
		sys_err("cannot find sectree by %dx%d mapindex %d for building", x, y, lMapIndex);
		return false;
	}
...
void CLand::PutData(const TLand* data)
{
	...
	if (m_data.dwGuildID)
	{
		...
		if (r)
		{
			...
			if (CHARACTER_MANAGER::instance().GetCharactersByRaceNum(LAND_AGENT, vec))
			{
...
void CManager::FinalizeBoot()
{
	...
	while (it2 != m_map_pkLand.end())
	{
		...
		CHARACTER_MANAGER::instance().SpawnMob(LAND_AGENT, r.lMapIndex, region->sx + r.x + (r.width / 2), region->sy + r.y + (r.height / 2), 0);
	}
}
