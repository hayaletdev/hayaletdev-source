...
namespace quest
{
	...
	int party_chat(lua_State* L)
	{
		...
		if (pParty)
		{
			FPartyChat f(CHAT_TYPE_PARTY, lua_tostring(L, 1));
			pParty->ForEachOnlineMember(f);
		}
	...
	int party_is_map_member_quest_flag_lt(lua_State* L)
	{
		...
		std::string questname = lua_tostring(L, 1);
		std::string questflag = lua_tostring(L, 2);

		if (pParty)
		{
			FPartyCheckFlagLt f;
			f.flagname = questname + "." + questflag;
	...
	struct FGiveBuff
	{
		...
		FGiveBuff(DWORD _dwType, POINT_TYPE _wApplyOn, POINT_VALUE _lApplyValue, DWORD _dwFlag, long _lDuration,
	...
	int party_give_buff(lua_State* L)
	{
		...
		POINT_TYPE wApplyOn = lua_tonumber(L, 2);
		POINT_VALUE lApplyValue = lua_tonumber(L, 3);
	...
	int party_get_member_count(lua_State* L)
	{
		...
		lua_pushnumber(L, pParty ? pParty->GetMemberCount() : 0);
		return 1;
	}
	...
	void RegisterPartyFunctionTable()
	{
		luaL_reg party_functions[] =
		{
			...
			{ "is_map_member_quest_flag_lt", party_is_map_member_quest_flag_lt },
			{ "get_member_pids", party_get_member_pids }, // 파티원들의 pid를 return
			{ "get_member_count", party_get_member_count }, // 파티원들의 pid를 return
			{ NULL, NULL }
		};
