#ifndef __INC_GUILD_MANAGER_H__
#define __INC_GUILD_MANAGER_H__
...
public:
	void SendGuildEventFlagList(LPCHARACTER pChar);

	void RequestSetEventFlag(DWORD dwGuildID, const std::string& strFlagName, long lValue);
	void SetEventFlag(DWORD dwGuildID, const std::string& strFlagName, long lValue);
	long GetEventFlag(DWORD dwGuildID, const std::string& strFlagName);

private:
	typedef std::map<std::string, long> TEventFlagMap;
	typedef std::map<DWORD, TEventFlagMap> TGuildEventFlagMap;
	TGuildEventFlagMap m_mapGuildEventFlag;
#endif
