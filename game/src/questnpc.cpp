...
namespace quest
{
	...
	bool NPC::HandleEvent(PC& pc, int EventIndex)
	{
		...
		if (fMatch.Matched())
		{
			for (int i = 0; i < fMatch.size; i++)
			{
				if (i != 0)
				{
					// 20120514 <김용욱> : 퀘스트 매니저의 m_pCurrentPC가 바뀌는 경우가 발생하여,
	...
	bool NPC::OnFishing(PC& pc) { return HandleReceiveAllEvent(pc, QUEST_FISHING_EVENT); }
	bool NPC::OnFishItem(PC& pc)
	{
		if (m_vnum == 0)
			return HandleReceiveAllEvent(pc, QUEST_FISH_ITEM_EVENT);
		else
			return HandleEvent(pc, QUEST_FISH_ITEM_EVENT);
	}
#endif
	...
	bool NPC::OnRestartHere(PC& pc)
	{
		return HandleReceiveAllEvent(pc, QUEST_RESTART_HERE_EVENT);
	}
#endif

#if defined(__QUEST_REQUEST_EVENT__)
	bool NPC::OnRequestEvent(PC& pc, DWORD quest_index, int state)
	{
		return ExecuteEventScript(pc, QUEST_REQUEST_EVENT, quest_index, state);
	}
#endif
}
