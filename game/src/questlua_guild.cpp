...
namespace quest
{
	...
	int guild_is_general(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::Instance().GetCurrentCharacterPtr();
	...
	int guild_get_near_count(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::Instance().GetCurrentCharacterPtr();

		if (ch && ch->GetGuild())
			lua_pushnumber(L, ch->GetGuild()->GetNearMemberCount(ch));
		else
			lua_pushnumber(L, 0);
	...
	int guild_set_event_flag(lua_State* L)
	{
		if (!lua_isstring(L, 1) || !lua_isnumber(L, 2))
			return 0;

		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (pChar && pChar->GetGuild())
			CGuildManager::instance().RequestSetEventFlag(pChar->GetGuild()->GetID(), lua_tostring(L, 1), static_cast<long>(lua_tonumber(L, 2)));

		return 0;
	}

	int guild_get_event_flag(lua_State* L)
	{
		if (!lua_isstring(L, 1))
			return 0;
		...
		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (pChar && pChar->GetGuild())
			lua_pushnumber(L, CGuildManager::instance().GetEventFlag(pChar->GetGuild()->GetID(), lua_tostring(L, 1)));
		else
			lua_pushnumber(L, 0);
...
#if defined(__GUILD_DRAGONLAIR_SYSTEM__) //&& defined(__GUILD_RENEWAL__)
			{ "is_general", guild_is_general },
			{ "get_near_count", guild_get_near_count },
#endif
#if defined(__GUILD_EVENT_FLAG__) //&& defined(__GUILD_RENEWAL__)
			{ "set_event_flag", guild_set_event_flag },
			{ "get_event_flag", guild_get_event_flag },
#endif
			{ NULL, NULL }
		};
