...
static const char* GetMotionFileName(TMobTable* mobTable, EPublicMotion motion)
{
	...
	if (fp != NULL)
	{
		...
		switch (motion)
		{
			...
			case MOTION_SPECIAL_6: field = "SPECIAL5"; break;
			case MOTION_SPECIAL_7: field = "SPECIAL6"; break;
			case MOTION_SPECIAL_8: field = "SPECIAL7"; break;
...
static void LoadSkillMotion(CMotionSet* pMotionSet, CMob* pMob, EPublicMotion motion)
{
	...
	switch (motion)
	{
		...
		case MOTION_SPECIAL_6: idx = 5; break;
		case MOTION_SPECIAL_7: idx = 6; break;
		case MOTION_SPECIAL_8: idx = 7; break;
		default:
			return;
	}
...
{
	std::map<DWORD, float>::iterator f = m_map_fNormalAttackDuration.find(dwVnum);
	if (m_map_fNormalAttackDuration.end() == f)
		return 0.0f;
	else
		return f->second;
}
...
bool CMotionManager::Build()
{
	...
	for (BYTE bRace = 0; bRace < MAIN_RACE_MAX_NUM; ++bRace)
	{
		CMotionSet* pkMotionSet = M2_NEW CMotionSet;
		m_map_pkMotionSet.emplace(bRace, pkMotionSet);

		char szBuf[256] = {};

		snprintf(szBuf, sizeof(szBuf), "%s/general/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_GENERAL, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/general/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_GENERAL, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/twohand_sword/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_TWOHAND_SWORD, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/twohand_sword/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_TWOHAND_SWORD, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/onehand_sword/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_ONEHAND_SWORD, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/onehand_sword/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_ONEHAND_SWORD, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/dualhand_sword/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_DUALHAND_SWORD, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/dualhand_sword/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_DUALHAND_SWORD, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/bow/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_BOW, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/bow/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_BOW, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/bell/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_BELL, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/bell/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_BELL, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/fan/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_FAN, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/fan/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_FAN, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/claw/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_CLAW, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/claw/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_CLAW, MOTION_WALK);

		snprintf(szBuf, sizeof(szBuf), "%s/horse/run.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_HORSE, MOTION_RUN);
		snprintf(szBuf, sizeof(szBuf), "%s/horse/walk.msa", c_apszFolderName[bRace]);
		pkMotionSet->Load(szBuf, MOTION_MODE_HORSE, MOTION_WALK);
	}
	...
	while (it != CMobManager::instance().end())
	{
		...
		if ('\0' != t->szFolder[0])
		{
			...
			LoadSkillMotion(pkMotionSet, pkMob, MOTION_SPECIAL_6);
			LoadSkillMotion(pkMotionSet, pkMob, MOTION_SPECIAL_7);
			LoadSkillMotion(pkMotionSet, pkMob, MOTION_SPECIAL_8);
			...
			m_map_fNormalAttackDuration.insert(std::map<DWORD, float>::value_type(t->dwVnum, normalAttackDuration));
			// END_OF_POLYMORPH_BUG_FIX
		}
	}
	...
	for (DWORD i = 0; i < rkTextFileLoader.GetChildNodeCount(); ++i)
	{
		...
		if (0 == strNodeName.compare("motioneventdata"))
		{
			...
			for (DWORD j = 0; j < dwMotionEventDataCount; ++j)
			{
				...
				// pMob->AddSkillSplash(iSkillIndex, 100 + DWORD(fStartingTime * 1000), (DWORD)std::abs(v3Position.y)); // used to be -v3Position.y (but it's unsigned!)
