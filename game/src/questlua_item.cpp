...
namespace quest
{
	...
	int item_remove(lua_State* L)
	{
		...
		if (item != NULL)
		{
			...
			{
				DWORD count = (DWORD)lua_isnumber(L, 1);
				if (count > 0)
	...
	int item_get_duration_limit(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();

		if (q.GetCurrentItem())
		{
			lua_pushnumber(L, q.GetCurrentItem()->GetDurationLimit());
			return 1;
		}
	...
	int item_copy_and_give_before_remove(lua_State* L)
	{
		...
		if (pkNewItem)
		{
			...
			pItem->GetRandomApplyTable(aApplyRandom, CApplyRandomTable::GET_CURRENT);
			if (aApplyRandom)
	...
	int item_get_set_value(lua_State* L)
	{
		CQuestManager& rQMgr = CQuestManager::instance();
		if (rQMgr.GetCurrentItem())
			lua_pushnumber(L, rQMgr.GetCurrentItem()->GetItemSetValue());
		else
			lua_pushnumber(L, 0);
		return 1;
	}
	...
	void RegisterITEMFunctionTable()
	{
		luaL_reg item_functions[] =
		{
			...
			{ "get_duration_limit", item_get_duration_limit },
...
#endif
#if defined(__SET_ITEM__)
			{ "get_set_value", item_get_set_value },
#endif
