...
namespace marriage
{
	...
	struct TMarriageItemBonusByGrade
	{
		...
		{ UNIQUE_ITEM_MARRIAGE_PENETRATE_BONUS, { 4, 5, 6, 8, } }, // 관통 증가
		{ UNIQUE_ITEM_MARRIAGE_EXP_BONUS, { 10, 12, 15, 20, } }, // 경험치 증가
		{ UNIQUE_ITEM_MARRIAGE_CRITICAL_BONUS, { 4, 5, 6, 8, } }, // 크리티컬 증가
		{ UNIQUE_ITEM_MARRIAGE_TRANSFER_DAMAGE, { -4, -5, -6, -8, } }, // 상대방 공격력 감소
		{ UNIQUE_ITEM_MARRIAGE_ATTACK_BONUS, { 20, 25, 30, 40, } }, // 공격력 증가 (절대값)
		{ UNIQUE_ITEM_MARRIAGE_DEFENSE_BONUS, { 12, 16, 20, 30, } }, // 방어력 증가 (절대값)
	...
	int TMarriage::GetMarriagePoint()
	{
		...
		if (IsOnline())
		{
			if (ch1->GetPremiumRemainSeconds(PREMIUM_MARRIAGE_FAST) > 0 ||
				...
				ch1->IsEquipUniqueGroup(UNIQUE_GROUP_MARRIAGE_FASTER_LOVE_POINT) ||
				ch2->IsEquipUniqueGroup(UNIQUE_GROUP_MARRIAGE_FASTER_LOVE_POINT))
			{
