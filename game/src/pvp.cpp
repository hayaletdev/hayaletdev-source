...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if ((pkPVP = Find(kPVP.m_dwCRC)))
	{
		...
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s님과의 대결 시작!", pkChr->GetName()));
			pkChr->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s님과의 대결 시작!", pkVictim->GetName()));
		}
		return;
	}
	...
	snprintf(msg, sizeof(msg), LC_STRING("%s님이 대결신청을 했습니다. 승낙하려면 대결동의를 하세요.", pkChr->GetName()));

	pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
	pkChr->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s에게 대결신청을 했습니다.", pkVictim->GetName()));
