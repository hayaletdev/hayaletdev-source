...
EVENTFUNC(battle_arena_event)
{
	...
	if (pInfo != NULL)
	{
		...
		{
			++pInfo->state;
			BroadcastNotice(LC_STRING("몬스터들의 공격까지 5분 남았습니다!!!"));
		}
		...
		{
			++pInfo->state;
			BroadcastNotice(LC_STRING("몬스터들의 공격까지 1분 남았습니다!!!"));
		}
		...
		case 2:
		{
			...
			BroadcastNotice(LC_STRING("몬스터들이 성을 공격하기 시작했습니다."));
		...
		case 3:
		{
			...
			{
				pInfo->state = 6;
				SendNoticeMap(LC_STRING("중앙 제단에 악의 기운이 모여듭니다."), pInfo->nMapIndex, false);
			}
			else
			{
				pInfo->wait_count++;
				...
				{
					pInfo->state++;
					SendNoticeMap(LC_STRING("몬스터들이 물러갈 조짐을 보입니다."), pInfo->nMapIndex, false);
				}
				else
				{
		...
		{
			pInfo->state++;
			SendNoticeMap(LC_STRING("몬스터들이 물러가기 시작했습니다."), pInfo->nMapIndex, false);
			SendNoticeMap(LC_STRING("잠시 후 마을로 돌아갑니다."), pInfo->nMapIndex, false);
		...
		case 6:
		{
			...
			SendNoticeMap(LC_STRING("몬스터들의 대장이 나타났습니다."), pInfo->nMapIndex, false);
			SendNoticeMap(LC_STRING("30분 내로 귀목령주를 물리쳐주세요."), pInfo->nMapIndex, false);
		...
		case 7:
		{
			if (SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0)
			{
				SendNoticeMap(LC_STRING("귀목령주와 그의 부하들을 모두 물리쳤습니다."), pInfo->nMapIndex, false);
				SendNoticeMap(LC_STRING("잠시 후 마을로 돌아갑니다."), pInfo->nMapIndex, false);
			...
			{
				SendNoticeMap(LC_STRING("귀목령주가 퇴각하였습니다."), pInfo->nMapIndex, false);
				SendNoticeMap(LC_STRING("잠시 후 마을로 돌아갑니다."), pInfo->nMapIndex, false);
...
bool CBattleArena::Start(int nEmpire)
{
	...
	char szBuf[1024];
	snprintf(szBuf, sizeof(szBuf), LC_STRING("%s의 성으로 몬스터들이 진군하고 있습니다.", EMPIRE_NAME(m_nEmpire)));
	BroadcastNotice(szBuf);
	BroadcastNotice(LC_STRING("10분 뒤 성을 공격할 예정입니다."));
