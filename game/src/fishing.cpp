...
namespace fishing
{
	...
	*/
...
#if defined(__FISHING_GAME__)
			ch->FindAffect(AFFECT_FISHING_GOLD_TUNA) ||
#endif
...
#if defined(__FISHING_GAME__)
		if (quest::CQuestManager::instance().GetEventFlag("fishing_game"))
		{
			if (fish_info[fish_idx].vnum == GOLDEN_TUNA_VNUM && ch->GetQuestFlag("golden_tuna.appear_delay") > get_global_time())
				return 0;
		}
#endif
	...
	void FishingSuccess(LPCHARACTER ch)
	{
		...
		if (quest::CQuestManager::instance().GetEventFlag("fishing_game"))
		{
			TPacketGCFishingGame FishingGamePacket(FISHING_GAME_SUBHEADER_GOAL);
			ch->GetDesc()->Packet(&FishingGamePacket, sizeof(FishingGamePacket));
		}
#endif
	}
	...
	void FishingFail(LPCHARACTER ch)
	{
		...
		if (quest::CQuestManager::instance().GetEventFlag("fishing_game"))
		{
			TPacketGCFishingGame FishingGamePacket(FISHING_GAME_SUBHEADER_QUIT);
			ch->GetDesc()->Packet(&FishingGamePacket, sizeof(FishingGamePacket));
		}
#endif
	}
	...
	void FishingStop(LPCHARACTER ch)
	{
		...
		// NOTE: We don't need to PacketAround because this function is called before
		// fishing happens and we only need to stop the fishing for the main character.
	...
	void FishingPractice(LPCHARACTER ch)
	{
		...
		if (rod && rod->GetType() == ITEM_ROD)
		{
			...
			if (rod->GetRefinedVnum() > 0 && rod->GetSocket(0) < rod->GetValue(2) && number(1, rod->GetValue(1)) == 1)
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("낚시대의 수련도가 증가하였습니다! (%d/%d)", rod->GetSocket(0), rod->GetValue(2)));
				if (rod->GetSocket(0) == rod->GetValue(2))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("낚시대가 최대 수련도에 도달하였습니다."));
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("어부를 통해 다음 레벨의 낚시대로 업그레이드 할 수 있습니다."));
				}
			}
...
#if defined(__FISHING_GAME__)
		if (ch->FindAffect(AFFECT_FISHING_GOLD_TUNA))
			return true;
#endif
	...
	EVENTFUNC(fishing_event)
	{
		...
		switch (info->step)
		{
			case 0: // 흔들리기 또는 떡밥만 날아감
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot fish while other windows are open."));
					FishingStop(ch);
					return 0;
				}
...
#if defined(__FISHING_GAME__)
				int iFishingGameLevel = quest::CQuestManager::instance().GetEventFlag("fishing_game");
				if (iFishingGameLevel != 0)
				{
					TPacketGCFishingGame FishingGamePacket(FISHING_GAME_SUBHEADER_OPEN, iFishingGameLevel);
					ch->GetDesc()->Packet(&FishingGamePacket, sizeof(FishingGamePacket));
					return (PASSES_PER_SEC(FISHING_GAME_DURATION));
				}
#endif
	...
	int GetFishingLevel(LPCHARACTER ch)
	{
		...
		return rod->GetValue(2) + rod->GetSocket(0);
	}

	int Compute(LPCHARACTER ch, DWORD fish_id, DWORD ms, DWORD* item, int level)
	{
		if (fish_id == 0)
			return -2;
		...
		if (quest::CQuestManager::instance().GetEventFlag("fishing_game"))
		{
			//int time_step = number(0, MAX_FISHING_TIME_COUNT);
			//int rnd_prob = aFishingTime[fish_info[fish_id].time_type][time_step];
			//
			//if (ch->FindAffect(PREMIUM_FISH_MIND))
			//	rnd_prob += 50;
			//
			//if (number(1, 100) <= rnd_prob)
			{
	...
	void Take(fishing_event_info* info, LPCHARACTER ch)
	{
		...
		if (info->step == 1) // 고기가 걸린 상태면..
		{
			...
			DWORD item_vnum = 0;
			int ret = Compute(ch, info->fish_id, ms, &item_vnum, GetFishingLevel(ch));
			...
			switch (ret)
			{
				...
				case -3: // 난이도 때문에 실패
				case -1: // 시간 확률 때문에 실패
					//ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("고기가 미끼만 빼먹고 잽싸게 도망칩니다."));
				{
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
				...
				case 0:
					//ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("고기가 잡혔습니다! (%s)", fish_info[info->fish_id].name));
					if (item_vnum)
					{
						FishingSuccess(ch);
						...
						if (item)
						{
							...
							if (test_server)
							{
								ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("이번에 잡은 물고기의 길이는 %.2fcm", item->GetSocket(0) / 100.f));
							}
							...
							if (quest::CQuestManager::instance().GetEventFlag("fishevent") > 0 && (info->fish_id == 5 || info->fish_id == 6))
							{
								...
								if (info->fish_id == 5)
								{
									strlcpy(p.szBoard, LC_STRING("낚시이벤트월척붕어"), sizeof(p.szBoard));
								}
								else if (info->fish_id == 6)
								{
									strlcpy(p.szBoard, LC_STRING("낚시이벤트잉어"), sizeof(p.szBoard));
								}

								db_clientdesc->DBPacket(HEADER_GD_HIGHSCORE_REGISTER, 0, &p, sizeof(TPacketGDHighscore));
							}

#if defined(__FISHING_GAME__) && defined(__QUEST_EVENT_FISH__)
							quest::CQuestManager::instance().FishItem(ch->GetPlayerID(), item);
#endif
						}
		...
		{
#if defined(__FISHING_GAME__)
			if (info->step > 1 && quest::CQuestManager::instance().GetEventFlag("fishing_game"))
				return;
#endif
			FishingPractice(ch);
		}
	...
	void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			DWORD item = 0;
			Compute(ch, fish_id, (number(2000, 4000) + number(2000, 4000)) / 2, &item, level);
		...
		for (std::map<std::string, int>::iterator it = fished.begin(); it != fished.end(); ++it)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s : %d 마리", it->first.c_str(), it->second));

		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%d 종류 %d 마리 낚음", fished.size(), total_count));
	}
		...
		else
		{
			...
			switch (fish_info[idx].used_table[u_index])
			{
				...
				case USED_WATER_STONE: // 2
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("고기가 흔적도 없이 사라집니다."));
					break;
				...
				case USED_SHELLFISH: // 1
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("배 속에서 조개가 나왔습니다."));
					ch->AutoGiveItem(SHELLFISH_VNUM);
					break;

				case USED_EARTHWARM: // 4
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("배 속에서 지렁이가 나왔습니다."));
	...
	void Grill(LPCHARACTER ch, LPITEM item)
	{
		...
		*/
		...
		// alive
		if (vnum >= 27803 && vnum <= 27832)
			idx = vnum - 27800;

		// dead
		if (vnum >= 27833 && vnum <= 27862)
			idx = vnum - 27830;
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s를 구웠습니다.", LC_ITEM(item->GetVnum())));
		item->SetCount(0);
...
#if defined(__REFINE_FISHINGROD_RENEWAL__)
