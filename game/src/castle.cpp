...
EVENTFUNC(castle_siege_event)
{
	...
	{
		snprintf(buf, sizeof(buf), LC_STRING("%s에서 봉화를 둘러싸고 전투가 진행중입니다.", EMPIRE_NAME(GET_SIEGE_EMPIRE())));
		BroadcastNotice(buf);
	...
	{
		snprintf(buf, sizeof(buf), LC_STRING("%s이 수성에 성공했습니다.", EMPIRE_NAME(GET_SIEGE_EMPIRE())));
		BroadcastNotice(buf);

		snprintf(buf, sizeof(buf), LC_STRING("지금부터 %s은 30분간 봉화를 파괴하여 보상을 획득 할 수 있습니다.", EMPIRE_NAME(GET_SIEGE_EMPIRE())));
		BroadcastNotice(buf);
	...
	{
		//BroadcastNotice(LC_STRING("30분이 경과했습니다.. 봉화가 사라집니다."));
		castle_end_siege();
	}
	break;
...
static long FN_castle_map_index(int empire)
{
	switch (empire)
	{
	case EMPIRE_RED: return CASTLE_RED;
	case EMPIRE_YELLOW: return CASTLE_YELLOW;
	case EMPIRE_BLUE: return CASTLE_BLUE;
	}
	return 0;
}
...
int castle_siege(int empire, int tower_count)
{
	// check siege map
	{
		if (NULL == SECTREE_MANAGER::instance().GetMap(CASTLE_RED)) return 0;
		if (NULL == SECTREE_MANAGER::instance().GetMap(CASTLE_YELLOW)) return 0;
		if (NULL == SECTREE_MANAGER::instance().GetMap(CASTLE_BLUE)) return 0;
	}
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	{
		char buf[1024];
		snprintf(buf, sizeof(buf), LC_STRING("%s에 전쟁의 시작을 알리는 봉화가 나타났습니다.", EMPIRE_NAME(empire)));
		BroadcastNotice(buf);
	}
	return true;
}
...
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	{
		int	siege_end = true;
		snprintf(buf, sizeof(buf), LC_STRING("%s이 봉화를 파괴했습니다.", EMPIRE_NAME(killer_empire)));
		BroadcastNotice(buf);
		...
		if (siege_end)
		{
			...
			{
				snprintf(buf, sizeof(buf), LC_STRING("%s이 수성에 실패하여 전쟁에 패배하였습니다..", EMPIRE_NAME(GET_SIEGE_EMPIRE())));
				BroadcastNotice(buf);
			}
			else
			{
				snprintf(buf, sizeof(buf), LC_STRING("%s이 모든 봉화를 파괴하였습니다.", EMPIRE_NAME(GET_SIEGE_EMPIRE())));
				BroadcastNotice(buf);
			}
			castle_end_siege();
		}
...
bool castle_is_my_castle(int empire, int map_index)
{
	switch (empire)
	{
	case EMPIRE_RED: return (CASTLE_RED == map_index);
	case EMPIRE_YELLOW: return (CASTLE_YELLOW == map_index);
	case EMPIRE_BLUE: return (CASTLE_BLUE == map_index);
	}
	return false;
}
