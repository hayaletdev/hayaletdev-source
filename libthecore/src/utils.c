...
int number_even_ex(int from, int to, const char* file, int line)
{
	int result = number_ex(from, to, file, line);
	while (result % 2 != 0 && result != from)
	{
		result = number_ex(from, to, file, line);
	}
	return result;
}
