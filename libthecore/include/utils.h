...
#define TOKEN(string) if (!str_cmp(token_string, string))
	...
	extern int number_even_ex(int from, int to, const char* file, int line);
#define number(from, to) number_ex(from, to, __FILE__, __LINE__)
#define number_even(from, to) number_even_ex(from, to, __FILE__, __LINE__)
...
#endif // __INC_LIBTHECORE_UTILS_H__
